...
export async function renderStatusDashboard(projectId?: string): Promise<string> {
...
export async function watchStatusDashboard(projectId?: string, intervalMs = 2000): Promise<void> {
  for (;;) {
...
function renderRows(rows: string[][], emptyText: string): string[] {
  if (rows.length === 0) {
    return [emptyText];
  }
