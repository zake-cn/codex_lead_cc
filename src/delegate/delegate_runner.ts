...
export async function runDelegate(options: DelegateOptions): Promise<DelegateResult> {
  ...
  if (loadedClaudeEnv.loaded) {
    ...
    for (const w of loadedClaudeEnv.warnings) log(`  env warning: ${w}`);
  }
  ...
  const running = startClaudeCli({
    ...
    env: finalClaudeEnv,
    onStdout(_chunk: string) { stdoutChunks++; if (stdoutChunks === 1) log("claude stdout started"); },
    onStderr(_chunk: string) { stderrChunks++; if (stderrChunks === 1) log("claude stderr started"); },
  });
...
function writeEnvDiagnostic(artifactDir: string, info: Record<string, unknown>): void {
  try {
    writeFileSync(
      path.join(artifactDir, "claude_env_applied.json"),
...
export async function delegateMain(rawArgs: string[]): Promise<void> {
  ...
  for (let i = 0; i < rawArgs.length; i++) {
    ...
    if (arg === "--task-file") { if (!next) throw new Error("--task-file requires a value."); taskFile = next; i++; }
    else if (arg === "--session-file") { if (!next) throw new Error("--session-file requires a value."); sessionFile = next; i++; }
    else if (arg === "--timeout-sec") { if (!next) throw new Error("--timeout-sec requires a value."); timeoutSec = Number(next); if (!Number.isInteger(timeoutSec) || timeoutSec <= 0) throw new Error("--timeout-sec must be a positive integer."); i++; }
    else if (arg === "--dry-run") { dryRun = true; }
    else if (arg === "--help" || arg === "-h") { process.stdout.write(delegateHelp()); return; }
    else { throw new Error(`Unknown delegate argument: ${arg}`); }
  }
  if (!taskFile) { taskFile = process.env.CODEX_LEAD_CC_TASK_FILE; if (!taskFile) throw new Error("--task-file is required."); }
  if (!sessionFile) { sessionFile = process.env.CODEX_LEAD_CC_SESSION_FILE; if (!sessionFile) throw new Error("--session-file is required."); }
...
function assertInside(child: string, parent: string, label: string): void {
  if (!isInside(child, parent)) throw new Error(`${label} must be inside supervisor_home.\n  ${label}: ${child}\n  supervisor_home: ${parent}`);
}
function assertOutside(child: string, parent: string, label: string): void {
  if (isInside(child, parent)) throw new Error(`${label} must NOT be inside supervisor_home.\n  ${label}: ${child}\n  supervisor_home: ${parent}`);
}

// ── Dry run ──

function dryRunResult(
  taskFile: ParsedTaskFile, session: SessionFile, prompt: string,
  artifactDir: string, envLoaded: boolean, missingCritical: string[],
): DelegateResult {
