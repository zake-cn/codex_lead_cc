...
import type { DelegateResult, FinalTaskStatus, SessionFile, WorkerType } from "../types.js";
...
type QueueResult = DelegateResult | ErrorResult;

export async function daemonMain(rawArgs: string[]): Promise<void> {
  const options = parseDaemonArgs(rawArgs);
  await runDaemon(options);
}

export async function submitMain(rawArgs: string[]): Promise<void> {
  try {
...
async function runDaemon(options: DaemonOptions): Promise<void> {
  ...
  const cleanup = () => {
    ...
    if (existsSync(readyFile)) {
      try { unlinkSync(readyFile); } catch { /* non-fatal */ }
    }
  };
  const exitCleanly = () => {
    cleanup();
...
async function processRequestFile(requestFile: string, session: SessionFile): Promise<void> {
...
async function submit(options: SubmitOptions): Promise<QueueResult> {
  ...
  while (Date.now() < deadline) {
    if (existsSync(resultFile)) {
      ...
      return JSON.parse(raw) as QueueResult;
    }
...
function writeRequest(queueDir: string, request: DelegateRequest): void {
...
function writeQueueResult(resultDir: string, requestId: string, result: QueueResult): void {
...
function writeCompactJson(result: QueueResult): Promise<void> {
...
function writeReadyFile(readyFile: string, sessionFile: string, session: SessionFile): void {
  writeFileSync(
    readyFile,
    JSON.stringify({
      pid: process.pid,
...
function daemonUnavailableReason(sessionFile: string, session: SessionFile): string | undefined {
  if (!session.daemon_pid) {
    return (
      `Delegate daemon is not registered in this session.\n` +
      `  session_file: ${sessionFile}`
    );
  }
  if (!isProcessAlive(session.daemon_pid)) {
    return (
      `Delegate daemon is not running for this session.\n` +
  ...
  if (!existsSync(readyFile)) {
    return (
      `Delegate daemon ready file is missing.\n` +
      `  daemon_pid: ${session.daemon_pid}\n` +
  ...
  try {
    ready = JSON.parse(readFileSync(readyFile, "utf8")) as Record<string, unknown>;
  } catch {
    return `Delegate daemon ready file is invalid: ${readyFile}`;
  }
  if (typeof ready.pid === "number" && ready.pid !== session.daemon_pid) {
    return (
      `Delegate daemon ready file pid does not match session daemon_pid.\n` +
      `  daemon_pid: ${session.daemon_pid}\n` +
      `  ready_pid: ${ready.pid}\n` +
      `  ready_file: ${readyFile}`
    );
  ...
  if (ageMs > HEARTBEAT_STALE_MS) {
    return (
      `Delegate daemon heartbeat is stale.\n` +
      `  daemon_pid: ${session.daemon_pid}\n` +
...
async function loadTaskInfo(taskFile: string, artifactRoot: string): Promise<{
  taskId: string;
...
function fallbackTaskInfo(requestId: string, artifactRoot: string): {
  taskId: string;
  workerType: WorkerType;
  artifactDir: string;
} {
...
function nextRequestFile(queueDir: string): string | undefined {
...
function parseRequest(raw: string, label: string): DelegateRequest {
  let parsed: unknown;
  try {
  ...
  const request = parsed as Partial<DelegateRequest>;
...
function parseDaemonArgs(rawArgs: string[]): DaemonOptions {
...
function parseSubmitArgs(rawArgs: string[]): SubmitOptions {
...
function assertSessionQueuePaths(session: SessionFile): void {
...
function requestIdFromQueueFile(requestFile: string): string {
  return path.basename(requestFile).replace(/\.json$/, "");
}

function readParentPid(): number | undefined {
