...
const NO_WAKE_EVENTS = new Set<string>([
  "worker_stdout_chunk",
  "heartbeat",
  "file_read",
  "stage_changed",
...
export function buildNotificationFromEvent(
  ...
  event: EventRecord,
): Omit<SupervisorNotificationRecord, "notification_id" | "read" | "created_at"> | undefined {
...
export function defaultWakeTypes(): WakeEventType[] {
  return [
...
export function comparePriority(a: WakePriority, b: WakePriority): number {
  return priorityRank(b) - priorityRank(a);
}

export function highestPriority(notifications: Array<{ priority: WakePriority }>): WakePriority {
  return notifications.reduce<WakePriority>(
    (highest, notification) =>
      priorityRank(notification.priority) > priorityRank(highest) ? notification.priority : highest,
    "low",
  );
}

export function priorityRank(priority: WakePriority): number {
...
function wakeTypeForEvent(state: AgentForemanState, event: EventRecord): WakeEventType | undefined {
...
function priorityForWake(type: WakeEventType, role?: WorkerRole): WakePriority {
...
function requiresAction(type: WakeEventType): boolean {
  return [
    "permission_requested",
    "task_completed",
    "task_failed",
...
function recommendedNextActions(type: WakeEventType, role?: WorkerRole): string[] {
...
function summaryForWake(type: WakeEventType, eventSummary: string, role?: WorkerRole): string {
  if (type === "patch_generated") {
...
function permissionRequestIdForEvent(event: EventRecord): string | undefined {
...
function patchIdForEvent(state: AgentForemanState, event: EventRecord): string | undefined {
...
function reportIdForEvent(event: EventRecord): string | undefined {
  if (!event.task_id) {
    return undefined;
  }
...
function planIdFromPayload(event: EventRecord): string | undefined {
