...
export class SessionManager {
  ...
  async getWorkerHealth(input: GetWorkerHealthInput): Promise<{ workers: WorkerHealthReport[] }> {
  ...
  async restartWorker(input: RestartWorkerInput): Promise<{
    worker_id: string;
  ...
  async cleanupIdleWorkers(input: CleanupIdleWorkersInput): Promise<{
    dry_run: boolean;
...
function buildHealthReport(worker: WorkerRecord): WorkerHealthReport {
  const lastActive = worker.last_active_at ?? worker.updated_at ?? worker.created_at;
...
function inferHealth(worker: WorkerRecord, idleForMs: number | null, idleTimeoutSec: number): WorkerHealthStatus {
