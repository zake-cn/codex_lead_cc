...
export class WorktreeManager {
  ...
  async prepareTaskExecution(task: TaskRecord): Promise<{
  ...
  async cleanup(input: CleanupWorktreeInput): Promise<{
    cleaned: string[];
  }> {
    const state = await this.store.readState();
    const tasks = Object.values(state.tasks).filter((task): task is TaskRecord & { worktree_path: string } => {
...
async function getGitInfo(projectPath: string): Promise<{ isGitRepo: boolean; baseBranch?: string }> {
  try {
