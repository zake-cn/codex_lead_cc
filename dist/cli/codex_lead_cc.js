...
import { ensureSupervisorFiles, formatSupervisorMigrationSummary, migrateSupervisorFiles, } from "../supervisor.js";
...
async function waitForBridgeReady(stateFile, sessionId, timeoutMs) {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
        if (isBridgeStateReady(stateFile, sessionId))
            return true;
        await sleep(100);
    }
...
function isBridgeStateReady(stateFile, sessionId) {
