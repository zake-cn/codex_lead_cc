...
export class SimpleAnsiTerminalScreen {
    ...
    snapshot() {
        const lastNonEmpty = Math.max(this.cursorRow, this.rows.reduce((last, line, index) => line.trim() ? index : last, 0));
    ...
    handleCsi(rawParams, final) {
        ...
        if (final === "A") {
            ...
            for (let i = 0; i < first; i++)
                this.rows.shift();
            while (this.rows.length < DEFAULT_ROWS)
                this.rows.push("");
        }
        else if (final === "T") {
            for (let i = 0; i < first; i++)
    ...
    writeText(value) {
        for (const char of value) {
            ...
            this.cursorCol++;
            if (this.cursorCol >= DEFAULT_COLS)
                this.newLine();
        }
    }
    newLine() {
