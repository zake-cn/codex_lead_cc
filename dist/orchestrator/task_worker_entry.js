...
async function finalizeTask(runtime, taskId, result) {
    const store = runtime.store;
    const state = await store.readState();
    ...
    const finalStatus = task.status === "stopped" ? "stopped" : result.status;
    let diffSummary;
...
async function failTaskBeforeRun(store, task, error) {
...
//# sourceMappingURL=task_worker_entry.js.map