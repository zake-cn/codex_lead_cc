...
export class PermissionEngine {
    ...
    async applyPermissionGate(task) {
...
function evaluateTaskPermission(task, rules) {
...
function classifyTask(role, taskText) {
    ...
    const environment = ["npm install", "pnpm install", "yarn add", "pip install"].find((pattern) => lower.includes(pattern));
    if (environment) {
        return {
            tool: "Bash",
            action: environment,
...
function defaultEffectForRisk(risk) {
    if (risk === "danger") {
        return "deny";
    }
...
function findMatchingRule(task, tool, action, rules) {
...
function inferTestCommand(taskText) {
    const match = taskText.match(/(python3? -m unittest[^\n;.]*|uv run pytest[^\n;.]*|pytest[^\n;.]*|npm test[^\n;.]*|pnpm test[^\n;.]*|yarn test[^\n;.]*)/i);
    return match?.[1]?.trim() ?? "run project tests";
}
function inferAffectedPaths(taskText) {
    const matches = taskText.match(/(?:src|tests|docs|examples)\/[A-Za-z0-9_./-]+/g);
    return Array.from(new Set(matches ?? []));
}
//# sourceMappingURL=permission_engine.js.map