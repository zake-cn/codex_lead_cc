...
export class TaskManager {
    ...
    processManager;
    scheduler;
    constructor(store, processManager, scheduler) {
        this.store = store;
    ...
    async getReport(input) {
        ...
        if (level === "summary") {
            return summarizeReportForSupervisor(fullReport);
        }
        if (level === "raw") {
    ...
    async writeInterimReport(taskId) {
...
function defaultWorktreeMode(role) {
    if (role === "implementer") {
        return "isolated";
    }
...
function linkTaskToPlan(state, task, planId, planTaskId) {
    const plan = planId ? state.plans[planId] : undefined;
    if (!plan && planId) {
        throw new Error(`Plan not found: ${planId}`);
    }
    const targetPlan = plan ?? Object.values(state.plans).find((candidate) => candidate.tasks.some((planTask) => planTask.plan_task_id === planTaskId));
    if (!targetPlan) {
        if (planTaskId) {
    ...
    if ((task.depends_on ?? []).length === 0 && planTask.depends_on?.length) {
        task.depends_on = [...planTask.depends_on]
            ...
            .filter((dependencyTaskId) => Boolean(dependencyTaskId));
...
function resolveReportTaskId(input, state) {
    if (input.task_id) {
...
function summarizeReportForSupervisor(report) {
    return {
...
function normalizeTimeout(timeoutSec = DEFAULT_TIMEOUT_SEC) {
